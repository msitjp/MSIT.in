...
class PrimaryNavigationMenu(models.Model):
    title = models.CharField(verbose_name="Link Title", max_length=50, help_text='Text to show in the Navigation')
    link = models.CharField(max_length=1000, help_text = 'Link to redirect to', blank=True, null=True)
    files = models.FileField(upload_to=image_name('title', 'navigations'), blank=True, null=True)
    ...
    def clean(self):
        ...
        if str(self.link) == "" and str(self.files) == "":
            raise ValidationError("Navigation menu must have either a link to redirect or a file attached to it")
        elif str(self.link) != "" and str(self.files) != "":
            raise ValidationError("Please enter either a link to redirect or a file to attach with the Menu item.")
        else:
            return self
...
class Faculty(models.Model):
    ...
    department = models.CharField(max_length=30, choices=DEPARTMENT, blank=True, null=True)
...
class LatestNews(models.Model):
    ...
    link = models.CharField(max_length=1000, help_text='Link for the News', blank=True, null=True)
    files = models.FileField(upload_to=image_name('title', 'news'), blank=True, null=True)
    ...
    additional_files = models.FileField(upload_to=image_name('title', 'news'), blank=True, null=True)
    created_at = models.DateTimeField(auto_now_add=True, auto_now=False)
    updated_at = models.DateTimeField(auto_now_add=False, auto_now=True)
    ...
    def clean(self):
        ...
        if str(self.link) == "" and str(self.files) == "":
            raise ValidationError("Latest News must have either a link to redirect or a file attached to it")
        elif str(self.link) != "" and str(self.files) != "":
            raise ValidationError("Please enter either a link to redirect or a file to attach with the News item.")
        else:
            if str(self.additional_title) == "":
                return self
            if str(self.additional_link) == "" and str(self.additional_files) == "":
              raise ValidationError("Addtional Title must have either a link to redirect or a file attached to it")
            elif str(self.link) != "" and str(self.files) != "":
                raise ValidationError("Please enter either a link to redirect or a file to attach with the Additional Title.")
            else:
                return self
    def save(self, *args, **kwargs):
        self.full_clean()
...
class Notice(models.Model):
    title = models.CharField(max_length=250, help_text='Text to display')
    link = models.CharField(max_length=1000, help_text='Link for the Notice', blank=True, null=True)
    files = models.FileField(upload_to=image_name('title', 'notices'), blank=True, null=True)
    new = models.BooleanField(default=True, help_text='This places a New Flag on th link')
    visible = models.BooleanField(default=True, help_text='Whether the News is Visible on Main Page')
    additional_title = models.CharField(max_length=50, help_text='Additional Link Title (if any)', blank=True, null=True)
    additional_link = models.CharField(max_length=1000, help_text='Additional Link (if any)', blank=True, null=True)
    additional_files = models.FileField(upload_to=image_name('title', 'notices'), blank=True, null=True)
    created_at = models.DateTimeField(auto_now_add=True, auto_now=False)
    updated_at = models.DateTimeField(auto_now_add=False, auto_now=True)
    ...
    def clean(self):
        ...
        if str(self.link) == "" and str(self.files) == "":
            raise ValidationError("A Notice must have either a link to redirect or a file attached to it")
        elif str(self.link) != "" and str(self.files) != "":
            raise ValidationError("Please enter either a link to redirect or a file to attach with the Notice.")
        else:
            if str(self.additional_title) == "":
                return self
            if str(self.additional_link) == "" and str(self.additional_files) == "":
...
class TimeTable(models.Model):
    ...
    department = models.CharField(max_length=30, help_text='Department Name', choices=DEPARTMENT, blank=True, null=True)
...
class Syllabus(models.Model):
    ...
    department = models.CharField(max_length=30, help_text='Department Name', choices=DEPARTMENT)
    semester = models.CharField(max_length=1, choices=SEMESTERS, blank=True, null=True)
